...
class DraggableItemView: NSView {
    ...
    private func itemRect() -> NSRect {
        return NSRect(origin: itemLocation, size: CGSize(width: 60, height: 20))
    }
    ...
    private func offsetItemLocationBy(x: CGFloat, y: CGFloat) {
        // TODO: Implement this method
        
        // Tell the display to redraw the old rect
        setNeedsDisplay(itemRect())
        
        // since the offset can be generated by both mouse moves and moveUp, moveDown, etc... actions, we'll invert the deltaY amount based on if the view is flipped or not.
        let invertDeltaY = CGFloat(isFlipped ? -1 : 1)
        
        itemLocation.x = itemLocation.x + x
        itemLocation.y = itemLocation.y + y * invertDeltaY
        
        // invalidate the new rect location so that it'll be redrawn
        needsToDraw(itemRect())
    }
    ...
    override func mouseDragged(with event: NSEvent) {
        ...
        if dragging {
            ...
            offsetItemLocationBy(x: newDragLocation.x - lastDragLocation!.x,
                             y: newDragLocation.y - lastDragLocation!.y)
            
            // Question about above - why wouldn't you just set the location of the item directly, instead of doing things by offsets? Perhaps this will be answered when I take a look at Listing 4-10
